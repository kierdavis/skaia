{ fetchurl, imageTools, stdenv }:

let
  # generate using: apt-get download --print-uris intel-opencl-icd
...
imageTools.customise {
  ...
  run = ''apt install -y /imgbuild/pkgs/*.deb && rm -f /var/cache/ldconfig/aux-cache /var/log/apt/history.log /var/log/apt/term.log /var/log/dpkg.log'';
