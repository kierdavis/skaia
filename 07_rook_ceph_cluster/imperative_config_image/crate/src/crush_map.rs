...
fn mutate(map: &str) -> String {
  lazy_static! {
    static ref OLD_CUSTOM_RULE: Regex = Regex::new(r"(?ims)^rule skaia.*^\}\n").unwrap();
  }
  let mut map = OLD_CUSTOM_RULE.replace_all(map, "").into_owned();
  let insert_pos = map.find("\n# end crush map").unwrap();
  let insert_content = CUSTOM_RULES;
  assert!(!insert_content.starts_with('\n'));
  assert!(insert_content.ends_with('\n'));
  assert!(!insert_content.ends_with("\n\n"));
  map.insert_str(insert_pos, insert_content);
  map
}
